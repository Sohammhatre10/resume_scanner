...
# Load your LLaMA model (you can specify the path to the model file)
llama = Llama(model_path="path_to_llama_model/llama-3.2.bin")
...
def analyze_resume(text):
    ...
    Resume Text: {text}
    """
    # Insert the actual resume text into the template
    prompt = template.format(text=text)

    # Generate the response using LLaMA model
    response = llama(prompt, max_tokens=512, stop=["}"])

    raw_output = response['choices'][0]['text']  # Extract the model output

    try:
        json_output = json.loads(raw_output)
    except json.JSONDecodeError:
        print("Error: The model output is not valid JSON. Raw output:", raw_output)
